...
if [ $PROCESS_NUM -eq 0 ];
        then
                ...
                nohup java -Xms4g -Xmx4g -XX:MaxGCPauseMillis=500 -cp .:config/:Pipeline-all-1.0.jar -Dlog4j.configuration=config/log4j.xml -Dprocess.name=skroll com.skroll.rest.WebServer --port 8088 --baseuri webapp 3>&1 1>/dev/null 2>&3- | tee logs/skroll.log&
